...
from src.api.schemas import (
    ...
    BOMResponse,
    BOMRiskResponse,
    BOMTransitionHistoryResponse,
    BOMTransitionRequest,
    BOMTransitionResponse,
    BOMUsageResponse,
    ComponentCreate,
    ComponentResponse,
)
...
@router.delete(
    "/{bom_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(verify_token)]
)
...
@router.post(
    "/{bom_id}/clone",
    response_model=BOMCloneResponse,
    status_code=status.HTTP_201_CREATED,
    dependencies=[Depends(verify_token)],
)
def clone_bom(bom_id: str, body: BOMCloneRequest, db: Neo4jClient = Depends(get_db)):
    ...
    return BOMCloneResponse(
        ...
        new_bom_id=body.new_bom_id,
        new_version=body.new_version,
        new_status=new_bom["status"],
        cloned_by=body.cloned_by or "system",
    )


@router.get(
    "/{bom_id}/diff/{other_bom_id}",
    response_model=BOMDiffResponse,
    dependencies=[Depends(verify_token)],
)
def diff_boms(bom_id: str, other_bom_id: str, db: Neo4jClient = Depends(get_db)):
    ...
    return BOMDiffResponse(
        bom_id_a=diff.bom_id_a,
        bom_id_b=diff.bom_id_b,
        ...
        modified=[
            {"part_id": c.part_id, "part_name": c.part_name, "changes": c.changes}
...
@router.get(
    "/{bom_id}/lineage", response_model=BOMLineageResponse, dependencies=[Depends(verify_token)]
)
...
@router.post(
    "/{bom_id}/approve", response_model=BOMApprovalResponse, dependencies=[Depends(verify_token)]
)
...
@router.post(
    "/{bom_id}/transition",
    response_model=BOMTransitionResponse,
    dependencies=[Depends(verify_token)],
)
...
@router.get(
    "/{bom_id}/transitions",
    response_model=BOMTransitionHistoryResponse,
    dependencies=[Depends(verify_token)],
)
